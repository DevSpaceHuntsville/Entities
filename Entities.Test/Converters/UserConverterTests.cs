...
namespace DevSpace.Common.Entities.Test {
	public class UserJsonConverterTests {
		...
		public void JsonDeserializer() {
			...
			Assert.Equal(
				expected: Enumerable.Range( 1, 3 ).Select( i => CreateUser( (byte)i ) ),
				actual: JsonConvert.DeserializeObject<IEnumerable<User>>( json )
			);
		}
		...
		public void JsonDeserializer_ItemsOutOfOrder() {
			...
			Assert.Equal(
				expected: CreateUser( 1 ),
				actual: JsonConvert.DeserializeObject<User>( json )
			);
		}
		...
		public void JsonSerializerFormattingNone() {
			...
			Assert.Equal(
				...
				actual: JsonConvert.SerializeObject(
					Enumerable.Range( 1, 3 ).Select( i => CreateUser( (byte)i ) )
				)
			);
		}
